...
deploy_stack.py
===============
Deploys the Lesson 11 Demo CloudFormation infrastructure using boto3.
Run this ONCE before the supply chain gateway demo.

No AWS CLI required — uses boto3 directly.

Usage:
    cd lesson-11-inter-agent-communication-via-gateway/demo-supply-chain-gateway/infrastructure
    python deploy_stack.py

What it creates:
    - Lambda functions  : lesson-11-demo-{inventory,shipping,supplier,quality-inspection}
    - IAM Role          : lesson-11-demo-agentcore-role  (for AgentCore Gateway)
...
STACK_NAME    = "lesson-11-demo-gateway"
PROJECT_NAME  = "lesson-11-demo"
...
def _print_outputs(stack: dict):
    ...
    print("\n" + "=" * 60)
    print("  Lesson 11 Demo Infrastructure — Ready")
    print("=" * 60)
    print(f"\n  AgentCore Gateway Role ARN:")
    print(f"    {role_arn}")
    print(f"\n  Lambda Functions:")
    for key in ["InventoryFunctionArn", "ShippingFunctionArn",
                "SupplierFunctionArn", "QualityInspectionFunctionArn"]:
    ...
    print(f"    python supply_chain_gateway.py\n")
