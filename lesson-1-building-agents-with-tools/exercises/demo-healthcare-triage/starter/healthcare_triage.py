...
Instructions:
  ...
  The tool functions are pre-written — focus on the agent setup.
...
def build_triage_agent() -> Agent:
    ...
    # TODO 1: Create a BedrockModel instance
    #   - Use MODEL_ID as the model_id
    #   - Set region_name to AWS_REGION
    #   - Set temperature to 0.0 for deterministic output
    pass

    # TODO 2: Write a system prompt that instructs the agent to:
    #   1. Call lookup_symptoms with the patient's complaint text
    #   2. Call classify_urgency with the EXACT JSON from lookup_symptoms
    #   3. Call book_appointment with patient_id and the "urgency" value from classify_urgency
    #   4. Write a brief 3-line summary after all tools complete
    #   IMPORTANT: Tell the agent to pass raw JSON between tools and use exact urgency values
    pass

    # ─── TOOL 1: Symptom Analysis (pre-written) ───
    ...
    # ─── TOOL 2: Urgency Classification (pre-written) ───
    ...
    # ─── TOOL 3: Appointment Scheduling (pre-written) ───
    ...
    # TODO 3: Create and return the Agent
    #   - Pass model, system_prompt, and tools=[lookup_symptoms, classify_urgency, book_appointment]
    pass
