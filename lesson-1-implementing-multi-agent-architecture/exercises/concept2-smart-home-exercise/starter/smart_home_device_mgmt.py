...
def build_device_manager() -> Agent:
    ...
    #   1. Call read_sensor_data with the device_id
    #   2. Call diagnose_issue with the EXACT JSON from read_sensor_data
    #   3. Call send_device_command with device_id and each issue_type from diagnosis
    #   4. Write a brief 3-line summary after all tools complete
    #   IMPORTANT: Tell the agent to pass raw JSON between tools and use exact issue values
    pass

    # ─── TOOL 1: Read Sensor Data (pre-written) ───
    ...
    # ─── TOOL 2: Diagnose Issue (pre-written) ───
    ...
    # ─── TOOL 3: Send Device Command (pre-written) ───
    ...
    #   - Pass model, system_prompt, and tools=[read_sensor_data, diagnose_issue, send_device_command]
    pass
